 ...
 * @param $file             Plugin file name.
 *
 * @return array
 */
function wyr_add_plugin_settings_link( $links, $file ) {
    $basename = wyr_get_plugin_basename();

    if ( is_plugin_active( $basename ) && $basename === $file ) {
        $links[] = '<a href="' . esc_url( wyr_admin_url( add_query_arg( array( 'page' => wyr_get_general_settings_page_id() ), 'admin.php' ) ) ) . '">'. esc_html__( 'Settings', 'wyr' ) .'</a>';
    }
 ...
 * @return array                    Output.
 */
function wyr_sanitize_text_array( $input_array ) {
