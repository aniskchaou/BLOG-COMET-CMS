...
function ma_add_plugin_settings_link( $links, $file ) {
    $basename = mace_get_plugin_basename();

    if ( is_plugin_active( $basename ) && $basename === $file ) {
        $pages = mace_get_settings_pages();
        $slugs = array_keys( $pages );

        $links[] = '<a href="' . esc_url( mace_admin_url( add_query_arg( array( 'page' => $slugs[0] ), 'admin.php' ) ) ) . '">'. esc_html__( 'Settings', 'mace' ) .'</a>';
    }
