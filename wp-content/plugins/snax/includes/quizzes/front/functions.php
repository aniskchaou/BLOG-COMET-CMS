...
function snax_quiz_enqueue_scripts() {
	...
	$config = array(
		...
		'questions_per_page' 		    => snax_get_questions_per_page(),
...
function snax_generate_quiz_pagination( $posts, $wp_query ) {
	...
	foreach ( $posts as $post ) {
		...
		if ( empty( snax_get_questions_per_page( $post ) ) ) {
			continue;
		}

		$questions = snax_get_questions_count( $post );
		$questions_per_page = snax_get_questions_per_page( $post );
