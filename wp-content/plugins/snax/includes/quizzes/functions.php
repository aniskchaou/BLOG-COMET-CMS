...
/**
 * Return number of questions per page.
 *
 * @param WP_Post $quiz		Optional. Post object or id.
 *
 * @return mixed            Number of questions or empty value for all questions
 */
function snax_get_questions_per_page( $quiz = null ) {
	$quiz = get_post( $quiz );

	// Legacy.
    $one_question_per_page = snax_get_quiz_setting( 'one_question_per_page', $quiz );

    if ( 'standard' === $one_question_per_page ) {
        update_post_meta( $quiz->ID, '_snax_questions_per_page', 1 );
        delete_post_meta( $quiz->ID, '_snax_one_question_per_page' );

    }

	return snax_get_quiz_setting( 'questions_per_page', $quiz );
}
...
function snax_is_quiz_last_page( $quiz = null ) {
    ...
    if ( empty( snax_get_questions_per_page( $quiz ) ) ) {
		return true;
	}
	...
	$questions		= (int) snax_get_questions_count( $quiz );
	$questions_per_page	= (int) snax_get_questions_per_page( $quiz );
	$last_page		= (int) round( $questions / $questions_per_page );
