...
function snax_handle_new_submission( $request ) {
	...
	if ( ! empty( $frontend_submission_nonce ) ) {
		...
		if ( 'list' === $format && ! empty( $list_type ) ) {
            ...
            $request->set_query_var( 'snax_list_type', $list_type );
		}
...
function snax_is_frontend_submission_page() {
    ...
    // Check only if the page was assigned.
    if ( $frontend_submission_page_id > 0 ) {
        $is = is_page( $frontend_submission_page_id );
    } else {
        $is = false;
    }
