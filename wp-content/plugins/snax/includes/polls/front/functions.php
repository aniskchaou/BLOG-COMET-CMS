...
function snax_poll_enqueue_scripts() {
	...
	$config = array(
		...
		'questions_per_page'           => snax_poll_get_questions_per_page(),
...
function snax_generate_poll_pagination( $posts, $wp_query ) {
	...
	foreach ( $posts as $post ) {
        ...
        if ( empty( snax_poll_get_questions_per_page( $post ) ) ) {
            continue;
        }

        $questions = snax_get_poll_questions_count( $post );
        $questions_per_page = snax_poll_get_questions_per_page( $post );
        $pages = round( $questions / $questions_per_page );
