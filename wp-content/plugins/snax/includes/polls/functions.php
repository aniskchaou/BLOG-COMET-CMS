 ...
 * @return bool
 */
function snax_poll_get_questions_per_page($poll = null ) {
	$poll = get_post( $poll );

    // Legacy.
    $one_question_per_page = snax_get_poll_setting( 'one_question_per_page', $poll );

    if ( 'standard' === $one_question_per_page ) {
        update_post_meta( $poll->ID, '_snax_questions_per_page', 1 );
        delete_post_meta( $poll->ID, '_snax_one_question_per_page' );

    }

    return snax_get_quiz_setting( 'questions_per_page', $poll );
}
...
function snax_is_poll_last_page( $poll = null ) {
    ...
    if ( empty( snax_poll_get_questions_per_page( $poll ) ) ) {
        return true;
    }
    ...
    $questions		= (int) snax_get_poll_questions_count( $poll );
    $questions_per_page	= (int) snax_poll_get_questions_per_page( $poll );
