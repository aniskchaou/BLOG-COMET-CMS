...
// Use custom label for term description.
add_filter( 'woocommerce_format_content',               'snax_extproduct_custom_term_description' );
...
function snax_create_extproduct( $data ) {
	...
	// $wc_prodcut should be properly casted to WC_Product_External but sometimes it isn't,
    // so let's manually do that.
    if ( ! $wc_product->is_type( 'external' ) ) {
        $wc_product = new WC_Product_External( $post_id );
    }
 ...
 * @return string
 */
function snax_extproduct_custom_term_description( $description ) {
