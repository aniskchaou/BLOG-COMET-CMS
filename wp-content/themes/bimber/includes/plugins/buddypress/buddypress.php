...
if ( version_compare( BP_VERSION, '6.0.0', '<' ) ) {
	...
	// BuddyPress v6.0.0+
	add_filter( 'bp_before_members_cover_image_settings_parse_args', 'bimber_cover_image_css', 10, 1 );
}
...
if ( function_exists( 'bp_set_theme_compat_feature' ) ) {
	$bimber_bp_components = array( 'groups' );

	if ( version_compare( BP_VERSION, '6.0.0', '<' ) ) {
		// Backward compatibility
		$bimber_bp_components[] = 'xprofile';
	} else {
		// BuddyPress v6.0.0+
		$bimber_bp_components[] = 'members';
	}

	bp_set_theme_compat_feature( 'legacy', array(
		'name'     => 'cover_image',
		'settings' => array(
			'components'   => $bimber_bp_components,
			'width'        => 1920,
			'height'       => 360,
